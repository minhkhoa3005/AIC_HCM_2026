...
def _resolve_keyframe_path(row: dict[str, Any], bundle_dir: Path) -> str:
    ...
    if root:
        ...
        # Metadata generated by video-search-agent commonly starts with
        # data/keyframes/, while the independent LLM folder stores only the
        # keyframe tree: Keyframes/<video_id>/<image>.
        parts = path.parts
        if len(parts) >= 3 and parts[0].lower() == "data" and parts[1].lower() == "keyframes":
            candidates.append(root_path.joinpath(*parts[2:]))
        if len(parts) >= 2:
            candidates.append(root_path.joinpath(*parts[-2:]))
